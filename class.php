...
class getinfo extends Tools_get{
	public function config(){
		include("config.php");
		$this->self = 'https://'.$_SERVER['HTTP_HOST'].preg_replace('/\?.*$/', '', isset($_SERVER['REQUEST_URI'])? $_SERVER['REQUEST_URI'] : $_SERVER['PHP_SELF']);
	...
	public function GetCookies($content){
		preg_match_all('/[Ss]et-[Cc]ookie: (.*);/U',$content,$temp);
		$cookie = $temp[1];
...
class stream_get extends getinfo {
    public function __construct(){
		...
		elseif (isset($_REQUEST['zip'])) $this->download_zip($_REQUEST['zip']);
		elseif (isset($_REQUEST['tgid']) && isset($_REQUEST['text'])) $this->sendTg($_REQUEST['tgid'],$_REQUEST['text']);
    ...
    public function download_zip($hash){   
		error_reporting (0);
		//$this->save_jobs();	
		if (! $job = $this->lookup_job($hash)) {
			sleep(15);
		...
		$link = '';
		$link = urldecode($job['url']);
		$filesize = $job['directlink']['cookies'];
		$filename = $job['filename'];
		$directlink = urldecode($job['directlink']['url']);
	 printf($this->lang['notice'],Tools_get::convert_time($this->ttl*60),$this->limitPERIP,Tools_get::convert_time($this->ttl_ip*60));
     die('<br>RFC: '.$filename.'<br> Folio: '.$filesize.'<br><br> pdf:  <a href="'.$link.'"  target="_blank">[Download]</a><br> xml: <a href="'.$directlink.'"  target="_blank">[Download]</a>');
     exit;
	}
	
	
	public function sendTg($id,$text){   
		error_reporting (0);
		//$this->save_jobs();	
		$url="https://api.telegram.org/bot".$this->tg_bot."/sendMessage?disable_web_page_preview=false&parse_mode=HTML&chat_id=".$id;	
 ...
 public function curl($url, $cookies, $post, $proxy, $header = 1, $json = 0, $ref = 0, $xml = 0, $h = NULL)
    {
        $ch = @curl_init();
        ...
        if ($header == -1) {
            ...
            curl_setopt($ch, CURLOPT_NOBODY, 1); // orig estaba en 1
        } else {
...
public function curl_cloud($url, $cookies, $post, $header = 1) {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, 10);
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.3');
    curl_setopt($ch, CURLOPT_HEADER, $header);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
...
public function curlz($url, $cookies = null, $post = null, $proxy = null, $header = 1, $json = 0, $ref = 0, $xml = 0, $h = NULL)
{
    $ch = @curl_init();
    if ($ch === false) {
    ...
    $options = [
        ...
        CURLOPT_USERAGENT => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.3',
        CURLOPT_REFERER => $ref === 0 ? $url : $ref,
    ...
    if ($cookies) {
        ...
        curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__).'/data/cookie.txt');
        curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__).'/data/cookie.txt');
    }
...
public function curl_tk($url, $cookies, $post, $proxy, $header = 1, $json = 0, $ref = 0, $xml = 0, $h = NULL)
    {
        $ch = @curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        if ($json == 1) {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, 30);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); // bywar mod
...
public function curlx($url, $cookies, $post, $proxy, $header=1)
    {
	$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $url);
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); // importante para kfe 1
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        if ($cookies) {
...
public function curl_by($url, $cookies, $post, $proxy, $header = 1, $json = 0, $ref = 0, $xml = 0, $h = NULL)
    {
        $ch = @curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        if ($json == 1) {
    ...
    if ($cookies) {
        ...
        curl_setopt($ch, CURLOPT_COOKIEFILE, __DIR__.'/'.$this->fileinfo_dir.'/cookie_'.$_SERVER['REMOTE_ADDR'].'.txt');
        curl_setopt($ch, CURLOPT_COOKIEJAR, __DIR__.'/'.$this->fileinfo_dir.'/cookie_'.$_SERVER['REMOTE_ADDR'].'.txt');
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Mobile Safari/537.36');
        curl_setopt($ch, CURLOPT_REFERER, $ref === 0 ? $url : $ref);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $head);
...
public function validate_imei($imei) {
	...
	public function GetCookies($content){
        ...
        preg_match_all('/[sS]et-[cC]ookie: (.*);/U',$content,$temp);        //bywarrior mod
		$cookie = $temp[1];
		$cookies = implode('; ',$cookie);
		return $cookies;
	}
	public function GetAllCookies($page) {
		...
		{
			preg_match('/[sS]et-[cC]ookie: (.*)/',$val,$temp);               //bywarrior mod
			if (isset($temp[1]))
			{
	...
	public function main(){
		...
		if($this->modvnl) {
			$checklog = false;
			$file = $this->fileinfo_dir."/log.txt";
			...
			if($checklog == false) {
				$data = $this->curl("https://ck.bywarrior.org/ipmod/ip/".$_SERVER['REMOTE_ADDR'], "", "", "",0);
				//$data = file_get_contents("http://vinaleech.com/ipmod/ip/".$_SERVER['REMOTE_ADDR']);
				if(stristr($data,"whitelist")==false) die("errorlogin");
				else {
					#-----------------------------------------------
		...
		else $dlhtml =  "Dead ❌ <a href=".$url." style='TEXT-DECORATION: none'><font color=red >".$url."</font></a> <img src=images/chk_error.png width='15' alt='errorlink'> <font color=#ffcc33>".$this->lang['errorlink']."</font><BR>";
		echo $dlhtml;
	}
	public function get($url){
		...
		if(!isset($filesize) or $filesize <=0 ) {                              // fixed bywarrior
		...
		if($this->direct==true)$ldir="<a href='$link'>Direct<a>";else{$ldir="";}
        if($this->bbcode==true) echo "<input name='1' type='text' size='100' value='[center][b][URL=".$lik."]".$this->title." [color=".$this->colorfn."]".$filename."[/color][color=".$this->colorfs. "] $msize [/color][/url][/b][/center]' onClick='1.select();' onfocus='this.select()'><br>"; 
		$dlhtml =  "<span class='shad'>Live ✅ <a title='click here to download' href='$lik' style='TEXT-DECORATION: none' target='$tiam'> <font color='#00CC00'>".$filename."</font> <font color='#FF66FF'>($msize)</font></a> $ldir</span><br>";
		return $dlhtml;
	}
...
class Tools_get{
	...
	public function size_namee($link,$cookie){
	...
	public function convert_name($filename){
		...
		if (empty($filename)==true) $filename =  substr(md5(time().$this->url), 0, 10);
		return $filename;
	}
	...
	public function report($url,$reason){
		if($reason == "dead"){
